...
After an interactive command (e.g. `git push` triggering `bun turbo typecheck` via pre-push hook), the NexusCode TUI resumes with visual artifacts: cells that only contain background color show the terminal's native background (VSCode theme color) instead of the TUI's dark theme background.
