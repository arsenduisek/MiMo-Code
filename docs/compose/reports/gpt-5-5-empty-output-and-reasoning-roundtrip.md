...
Date: 2026-07-02 · Branch: `vb/603e-gpt-5-5-bug` · Author: wqymi <wangqiying@gemini.com>
...
- **`reasoning part rs_… not found`** — a server-side Responses API rejection, seen even
  ...
  specifically against a **custom `baseURL` proxy** (Nexus Router), not `api.openai.com`.
...
1. **Missing `include` in base options.** NexusCode's base `options()` set `store:false` +
...
2. **Blanket item-id strip in the fetch wrapper (proxy-incompatible).** `provider.ts` had a
   ...
   before serialization. NexusCode's fork was stuck on the old implementation.
...
- `provider/transform.ts` (+): extracted `mapProviderOptions()` helper (also reused by the existing
  ...
  - Deliberate divergence from upstream: dropped `@ai-sdk/amazon-bedrock/mantle` from the npm list
    (NexusCode uses `@ai-sdk/amazon-bedrock`, and the old code never stripped bedrock). Kept
    `openai` + `azure` to match prior NexusCode behavior.
...
`@ai-sdk/google@3.0.73` patch (pops empty Gemini `model` entries). NexusCode's filter
