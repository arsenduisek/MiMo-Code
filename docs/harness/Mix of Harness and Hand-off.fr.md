...
Les tâches exigent des structures d'orchestration différentes. MoH prend actuellement en charge les cinq modes suivants. **Fallback est le mode par défaut de NexusCode** : c'est le chemin qui active automatiquement la détection Try-Best et le Hand-off.
...
NexusCode essaie d'abord d'effectuer le travail lui-même. Après un signal d'échec/blocage, l'utilisateur choisit un autre harness. Règles courantes :
...
- **Plan de contrôle = session d'origine** : panneau des tâches, routage des approbations, contexte et mémoire restent dans NexusCode ; le harness n'est qu'un sous-processus lancé.
...
- **Variable d'environnement `NEXUSCODE_ENABLE_TRY_BEST_HANDOFF`** (`true` par défaut)
...
Chaque seuil peut être remplacé dans `nexus.json` / config :
