...
**In one line**: package **Codex CLI** and **Claude Code CLI** as callable executors exposed through skills, so that when NexusCode falls into a "low-yield loop," it can pause the current turn and let the user hand the work to another harness with one action. The control plane remains in the NexusCode session, while the execution plane runs in the selected harness—the two planes are decoupled.
...
Different tasks need different orchestration structures. MoH currently supports the following five modes. **Fallback is NexusCode's default mode**—the path that automatically enables Try-Best detection and Hand-off.
...
NexusCode first attempts the work itself. After a failure or stall signal is detected, the user chooses another harness to take over. Common rules for detecting failure/stall include:
...
- **Control plane = original session**: the task panel, approval routing, context, and memory remain in the NexusCode session; the harness is only a launched subprocess.
...
- **Environment variable `NEXUSCODE_ENABLE_TRY_BEST_HANDOFF`** (default: `true`)
...
Detection thresholds can be overridden individually in `nexus.json` / config:
