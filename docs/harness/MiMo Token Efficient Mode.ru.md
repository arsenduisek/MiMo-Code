...
- **Один флаг, по умолчанию выключен** — `NEXUSCODE_EXPERIMENTAL_TOKEN_EFFICIENCY` — единственный переключатель для входа в пайплайн очистки, по умолчанию отключён; иначе вывод идёт напрямую по пути Raw.
...
**Один флаг, по умолчанию выключен** — `NEXUSCODE_EXPERIMENTAL_TOKEN_EFFICIENCY` — независимый флаг-переключатель, по умолчанию отключён, не выводится из `NEXUSCODE_EXPERIMENTAL=1`. Явный opt-in избавляет от тихого изменения поведения по умолчанию.
