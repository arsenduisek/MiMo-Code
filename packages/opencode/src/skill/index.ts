...
const NEXUSCODE_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  if (!Flag.NEXUSCODE_DISABLE_BUILTIN_SKILLS) {
    ...
    if (builtinSkillRoot && (yield* fsys.isDir(builtinSkillRoot))) {
      ...
      if (Flag.NEXUSCODE_DISABLE_OFFICIAL_SKILLS) {
  ...
  if (!Flag.NEXUSCODE_DISABLE_COMPOSE_SKILLS) {
  ...
  if (!Flag.NEXUSCODE_DISABLE_EXTERNAL_SKILLS) {
    const externalDirs = EXTERNAL_DIRS.filter((dir) => {
      if (dir === ".claude" && Flag.NEXUSCODE_DISABLE_CLAUDE_CODE_SKILLS) return false
      if (dir === ".codex" && Flag.NEXUSCODE_DISABLE_CODEX_SKILLS) return false
      if (dir === ".opencode" && Flag.NEXUSCODE_DISABLE_OPENCODE_SKILLS) return false
      return true
    })
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, NEXUSCODE_SKILL_PATTERN)
  }
