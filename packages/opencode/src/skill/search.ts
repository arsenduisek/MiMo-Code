  ...
  return normalize(value)
    ...
    .flatMap((token) => {
      if (!/^\p{Script=Han}+$/u.test(token)) {
        return token.length > Flag.NEXUSCODE_SKILL_SEARCH_STEM_MIN_LENGTH &&
          token.endsWith("s") &&
          !token.endsWith("ss")
          ? token.slice(0, -1)
          : token
...
export function searchSkills(query: string, skills: Skill.Info[]): SearchResult[] {
  ...
  const exact = searchable
    ...
    .map((skill) => ({
      skill_id: skill.name,
      name: skill.name,
      score: Flag.NEXUSCODE_SKILL_SEARCH_EXACT_SCORE,
  ...
  const scores = documents.map((document) =>
    queryTokens.reduce((score, token) => {
      ...
      const inverseDocumentFrequency = Math.log(
        1 +
          (documents.length - documentFrequency + Flag.NEXUSCODE_SKILL_SEARCH_BM25_IDF_SMOOTHING) /
            (documentFrequency + Flag.NEXUSCODE_SKILL_SEARCH_BM25_IDF_SMOOTHING),
      )
      return (
        score +
        inverseDocumentFrequency *
          ((frequency * (Flag.NEXUSCODE_SKILL_SEARCH_BM25_K1 + 1)) /
            (frequency +
              Flag.NEXUSCODE_SKILL_SEARCH_BM25_K1 *
                (1 -
                  Flag.NEXUSCODE_SKILL_SEARCH_BM25_LENGTH_NORMALIZATION +
                  Flag.NEXUSCODE_SKILL_SEARCH_BM25_LENGTH_NORMALIZATION * (document.length / averageLength))))
      )
    }, 0),
  )
  ...
  const bm25 = searchable
    ...
    .map((item) => ({
      ...
      score: Number(
        (
          (item.score / maximum) * Flag.NEXUSCODE_SKILL_SEARCH_BM25_SCORE_WEIGHT +
          item.coverage * Flag.NEXUSCODE_SKILL_SEARCH_QUERY_COVERAGE_WEIGHT
        ).toFixed(Flag.NEXUSCODE_SKILL_SEARCH_SCORE_PRECISION),
      ),
  ...
  return [...exact, ...bm25.filter((result) => !exactIDs.has(result.skill_id))].slice(
    0,
    Flag.NEXUSCODE_SKILL_SEARCH_MAX_RESULTS,
  )
}
