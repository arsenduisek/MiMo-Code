...
> **Bundled runtime:** when the `NEXUS_PYTHON` environment variable is set, skip `uv`/`python3` and pip installs entirely — run every command below with `uv run` replaced by `"$NEXUS_PYTHON"` (e.g. `"$NEXUS_PYTHON" scripts/extract_text.py input.docx`). This skill's Python dependencies are preinstalled in that interpreter; pip console scripts are unavailable, so always go through `"$NEXUS_PYTHON" -m <module>`. A bundled LibreOffice is exposed as `NEXUS_SOFFICE` and picked up automatically by the scripts here.
