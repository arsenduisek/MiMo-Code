---
name: drive-nexus
description: Use when you need to programmatically drive another NexusCode (nexus) process — supports both headless `nexus run` with JSON events and interactive TUI via tmux for full terminal interaction testing. Covers driving either an installed `nexus` binary or a dev build launched from source with `bun dev` (for debugging nexus itself). Reach for it to script, test, or automate a separate nexus instance and validate its behavior from parseable evidence.
---
...
Two **interfaces** for driving a separate nexus process:

| Interface | Command | Use when |
|------|---------|----------|
| **Headless** | `nexus run --format json` | Scripted tasks, CI, event validation |
| **TUI** | `nexus` in tmux | Interactive flow, permission dialogs, keybindings, visual regression |

**Core principle:** Every run produces parseable evidence. No eyeballing.

Always drive an *isolated* instance: give each run a fresh `NEXUSCODE_HOME` and a throwaway workspace so it never touches your own config, memory, or session DB.
...
|----------|---------|----------|
| **Installed binary** | `nexus …` | Testing a released/installed build |
| **Dev (from source)** | `bun dev …` | Debugging nexus itself — runs `src/index.ts` directly, no build step, picks up local code changes |

Everything below uses `nexus` for brevity. To drive a **dev build** instead,
substitute `bun dev` for `nexus` and run from the repo root — every flag,
JSON event, and tmux technique is identical. See the Dev Mode section.

## Prerequisites

```bash
# nexus binary must be on PATH (installed-binary launcher)
which nexus || echo "nexus not found on PATH"

# OR: dev launcher — run from the nexus repo root, needs bun
...
## Dev Mode (debugging nexus itself)

When the goal is to debug **nexus's own code**, launch from source with
`bun dev` instead of the installed `nexus` binary. It runs
...
- It is the dev equivalent of the `nexus` command: same CLI, same subcommands
...
- Args pass straight through, so **both interfaces work under dev**:
  ...
  - TUI:      `bun dev <workspace>` (positional workspace arg, as with `nexus`)
- If `NEXUSCODE_HOME` is not set, dev defaults it to a repo-local `.dev-home`
  dir. For an isolated driven run, set `NEXUSCODE_HOME=$(mktemp -d)` explicitly
  just like with the binary.

**Substitution rule:** anywhere Part 1 / Part 2 / Part 3 below say `nexus`,
replace it with `bun dev` (invoked from the repo root) to drive a dev build.

```bash
# Headless, dev build (from repo root)
REPO=/path/to/nexus/checkout   # your local nexus repo root
NEXUSCODE_HOME=$(mktemp -d) bun --cwd "$REPO" dev run \
  --format json --dangerously-skip-permissions --dir "$WORKSPACE" \
  < "$PROMPT" > /tmp/nexus-dev.jsonl 2>&1

# TUI, dev build in tmux (from repo root)
tmux new-session -d -s "$SESSION" -x 120 -y 30 \
  "cd $REPO && NEXUSCODE_HOME=$(mktemp -d) NEXUSCODE_PURE=true bun dev $WORKSPACE; sleep 999"
```

---

## Part 1: Headless Mode (`nexus run`)

### Launch

```bash
PROMPT=$(mktemp -t nexus-drive.XXXXXX)
cat >"$PROMPT" <<'EOF'
Your task here.
EOF

NEXUSCODE_HOME=$(mktemp -d) nexus run \
  --format json \
  --dangerously-skip-permissions \
  --dir "$WORKSPACE" \
  < "$PROMPT" > /tmp/nexus-out.jsonl 2>&1
...
# No errors
grep -q '"type":"error"' /tmp/nexus-out.jsonl && echo "FAIL: errors found"

# A specific tool was used (match part.tool, the bare string)
grep -q '"tool":"write"' /tmp/nexus-out.jsonl || echo "FAIL: write tool not called"

# Text output contains expected string (text is at .part.text)
grep '"type":"text"' /tmp/nexus-out.jsonl | jq -r '.part.text' | grep -q "expected"

# Robust tool check via jq (works regardless of key ordering)
jq -e 'select(.type=="tool_use") | .part.tool=="write"' /tmp/nexus-out.jsonl >/dev/null \
...
```bash
timeout 120 nexus run --format json --dangerously-skip-permissions < "$PROMPT"
...
| Variable | Purpose |
|---|---|
| `NEXUSCODE_HOME` | **Required.** Fresh `mktemp -d` per run. Sandboxes DB, config, cache. |
| `NEXUSCODE_PURE=true` | Disable external plugins. |
| `NEXUSCODE_DISABLE_GIT=true` | Skip git ops if workspace isn't a real repo. |
...
# Launch nexus TUI in tmux (workspace is a positional arg, NOT --dir)
tmux new-session -d -s "$SESSION" -x 120 -y 30 \
  "NEXUSCODE_HOME=$MHOME NEXUSCODE_PURE=true nexus $WORKSPACE; sleep 999"
...
test_smoke() {
  ...
  echo "Say hello" > "$P"
  NEXUSCODE_HOME=$MHOME nexus run --format json --dangerously-skip-permissions \
    < "$P" > /tmp/s1.jsonl 2>&1
  local E=$?
  rm -rf "$MHOME" "$P"
...
test_tool_use() {
  ...
  NEXUSCODE_HOME=$MHOME nexus run --format json --dangerously-skip-permissions --dir "$WS" \
    < "$P" > /tmp/s2.jsonl 2>&1
  local E=$?
  local OK=true
  [ $E -ne 0 ] && OK=false
...
test_tui_interactive() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="nexus-s3-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "NEXUSCODE_HOME=$MHOME NEXUSCODE_PURE=true nexus $WS; sleep 999"
...
```bash
test_tui_permission() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="nexus-s4-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "NEXUSCODE_HOME=$MHOME NEXUSCODE_PURE=true nexus $WS; sleep 999"
...
test_tui_keybindings() {
  local MHOME=$(mktemp -d) WS=$(mktemp -d) SID="nexus-s5-$$"
  local PROMPT_RE='>|Ask|/[a-z]'
  tmux new-session -d -s "$SID" -x 120 -y 30 "NEXUSCODE_HOME=$MHOME NEXUSCODE_PURE=true nexus $WS; sleep 999"
...
| Task | Command |
|------|---------|
| Headless run | `NEXUSCODE_HOME=$(mktemp -d) nexus run --format json --dangerously-skip-permissions "prompt"` |
| TUI launch | `tmux new-session -d -s test -x 120 -y 30 "NEXUSCODE_HOME=$(mktemp -d) nexus $WORKSPACE; sleep 999"` |
