...
- `~/.config/nexus/tui/*.{ts,tsx,js}` — global
- `<project>/.nexus/tui/*.{ts,tsx,js}` — project-local

Plugins can also be declared in `tui.json` (global `~/.config/nexus/tui.json`
or project `.nexus/tui.json`) via the `plugin` array (file URLs or npm specs).
...
```ts
// The full @nexus-code/sdk client — same as what plugins get on server side
