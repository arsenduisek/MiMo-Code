...
Before creating: check whether the extension already exists (`ls .nexus/tools .nexus/hooks .nexus/skills`). Prefer improving an existing one over adding a near-duplicate.
...
Need to redo a multi-agent run  → workflow (.nexus/workflows/*.js)
Need to change the UI           → TUI plugin (.nexus/tui/*.tsx)
```
...
Write to `.nexus/hooks/<name>.ts` — export a Hooks object:
...
Write to `.nexus/skills/<name>/SKILL.md`:
...
|------|------|-----------|
| Tools | `.nexus/tools/*.ts` | next turn |
| Hooks | `.nexus/hooks/*.ts` | next turn |
| Skills | `.nexus/skills/*/SKILL.md` | next turn |
| Workflows | `.nexus/workflows/*.js` | on invoke |
| TUI | `.nexus/tui/*.tsx` | restart |
