# NexusCode Permissions Reference
...
`mcp_sampling` gates MCP **client-side sampling**: an MCP server asking NexusCode to run a model call on its behalf (`sampling/createMessage`), so the server needs no API key of its own. The glob argument is the MCP server name, so you can allow one server and keep the rest at `ask`:

```jsonc
{
  "permission": {
    "mcp_sampling": { "*": "ask", "nexus-cut": "allow" }
  }
}
```
...
| Surface | How |
|---------|-----|
| TUI (`nexus`) | `nexus --dangerously-skip-permissions` |
| TUI at runtime | `/skip-permissions` — toggle mid-session, instance-wide, inherited by subagents |
| Headless (`nexus run`) | `nexus run --dangerously-skip-permissions "<prompt>"` |
| Any surface (env) | `NEXUSCODE_PERMISSION='"allow"'` or `NEXUSCODE_DANGEROUSLY_SKIP_PERMISSIONS=1` |

The `/skip-permissions` toggle auto-allows permission asks but keeps `deny` rules in force; forced-ask operations (e.g. destructive bash) still prompt and auto-reject after 60s (`NEXUSCODE_SKIP_ALL_FORCED_ASK_TIMEOUT_MS`) with feedback the model can act on, so unattended runs don't hang.
...
- A top-level catch-all like `"permission": { "*": "ask" }` makes the TUI/env form a no-op (your `*: ask` outranks the injected `*: allow`). Remove it, or use `nexus run --dangerously-skip-permissions`, which auto-replies at the event layer and overrides `ask` too.
...
- `external_directory` governs reads/writes outside the project working directory — by default these prompt, so NexusCode never silently widens scope.
