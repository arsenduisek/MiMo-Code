...
NexusCode implements the MCP client `sampling` capability
([spec](https://modelcontextprotocol.io/specification/2025-11-25/client/sampling)).
An MCP server can ask NexusCode to run a model call on its behalf via
...
The Nexus Cut MCP server extracts a video's audio track to a 16 kHz mono WAV and
needs a transcript. Instead of shipping its own provider credentials, it sends
the WAV to NexusCode as MCP `AudioContent` and asks for a completion:
...
```json
{
  ...
  "params": {
    ...
    "maxTokens": 2048,
    "modelPreferences": { "hints": [{ "name": "nexus-v2.5" }] }
  }
}
```

NexusCode picks a configured, audio-capable model (preferring the hinted
`nexus-v2.5`), asks the user to approve, runs the call, and returns the model's
text unchanged. `NEXUS_API_KEY` is never read or needed by the server.

## Capability negotiation

NexusCode declares `capabilities.sampling = {}` during `initialize`.
...
NexusCode does exactly that. `includeContext` defaults to `none`: NexusCode never
...
When nothing is eligible, NexusCode returns a **structured error** naming every
...
NexusCode is. It fails with `sampling stalled: the model produced no output` and
...
number.** Set `chunkTimeout` for a provider in `nexus.json` and it governs
...
Reaching this bound aborts the provider call as well as NexusCode's wait
...
While the model call is in flight NexusCode emits periodic
...
SDK's 60 s default gives up at 60 s while NexusCode is still working, since
NexusCode imposes no deadline of its own on the model call.
...
- **It reports observed output, but it is still not a fraction.** The model call
  ...
  actually produced — which is what lets a server tell "NexusCode is alive" from
...
- **It never carries the model's text.** Partial content is deliberately withheld:
  ...
  channel, and treating `message` as one would be NexusCode deciding that on the
...
- **It is necessary, not sufficient.** Resetting the timer is the *requester's*
  ...
  its own deadline. NexusCode cannot make that decision for it.
- **It cannot extend NexusCode's own bounds.** The notifications reset the peer's
...
NexusCode reads it back off the request's `_meta`. No token means no notifications
at all.
...
A sampling request normally arrives **while NexusCode is still waiting for that
...
2. NexusCode runs the work on a fresh root Effect fiber, which shares no fiber,
...
>
> NexusCode does not work around this, because it cannot: the id at risk belongs to
...
> it issues on a connection, NexusCode does not learn of it, so that request keeps a
> model call running — and a paid one — until one of NexusCode's own bounds aborts
...
- The model's text is returned **verbatim**. NexusCode does not summarise or
  rewrite it.
...
that is now the only deadline NexusCode imposes; the `"approval"`, `"model"` and
