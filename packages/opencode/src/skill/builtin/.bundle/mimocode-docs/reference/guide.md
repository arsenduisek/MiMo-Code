...
1. **Sign in** — `nexus account login <url>` runs a device flow: it prints a URL + code and opens your browser. `/connect` does the same from inside the TUI (e.g. to add OpenRouter). OAuth logins are available for Nexus Nexus, Codex (ChatGPT Pro/Plus), and xAI (Grok); Claude Code auth can be imported from `~/.claude/settings*.json`. Other account subcommands: `logout`, `switch`, `orgs`, `open`, `console`.
2. **Pick a model** — set `"model": "provider/model"` in config, or switch live in the TUI model dialog. Provider API keys are auto-detected from environment variables (unless `NEXUSCODE_NEXUS_ONLY=1`).
3. **List what's available** — `nexus models`, `nexus providers`.
...
**macOS default terminal** — NexusCode does not support the built-in Terminal.app. For misaligned output, flicker, or other rendering problems, use the VS Code integrated terminal or install iTerm2:
...
## Memory: making NexusCode remember
...
Drop a markdown file at `.nexus/command/<name>.md` (or `.nexus/commands/`, `.claude/command(s)/` are also read). The frontmatter configures it; the body is the prompt template.
...
A "mode" is just a **primary agent** with its own system prompt. To give NexusCode a custom mode (e.g. a `general` chat mode alongside the coding-focused `build`), drop a markdown file — no code, no server changes. The frontmatter is config; the **markdown body becomes the agent's system prompt**.
...
| Path | Scope |
|------|-------|
| `.nexus/agent/<name>.md` (or `agents/`) | project agent — most common |
| `.nexus/mode/<name>.md` (or `modes/`) | project mode — same as an agent forced to `mode: primary` |
| `~/.config/nexus/agent/<name>.md` | global agent, available in every project |
...
Verify a file loaded with `nexus agent list` — your agent shows up with its `(primary)` / `(subagent)` mode.
...
Inspect/manage with `nexus mcp`. Request timeout defaults to 5000ms (`timeout` per server, or `experimental.mcp_timeout` globally).

## Compose mode

Compose is NexusCode's specs-driven spec→ship lifecycle. Two interactive paths:
...
**Cron** schedules a prompt to be injected into a session on a recurring or one-shot basis. It is driven by the `cron` tool (no `/cron` slash command); `NEXUSCODE_EXPERIMENTAL_CRON` is **on by default** (kill switch: `NEXUSCODE_DISABLE_CRON`).
...
- **Durable** jobs persist to `<project>/.nexus/scheduled_tasks.json` and survive restart; non-durable jobs live only for the session.
...
To add tools, hooks, or skills, use the `evolve` skill — it covers writing `.nexus/tools/*.ts`, `.nexus/hooks/*.ts`, and `.nexus/skills/*/SKILL.md`, all hot-reloaded on the next turn.
