...
> **Bundled runtime:** when the `NEXUS_PYTHON` environment variable is set, skip the installs below — run every command with `python3`/`uv run` replaced by `"$NEXUS_PYTHON"` (pypdf/pypdfium2/reportlab/Pillow preinstalled; pip console scripts unavailable, use `"$NEXUS_PYTHON" -m <module>`). A bundled qpdf is exposed as `NEXUS_QPDF` (picked up automatically by the scripts here); invoke it directly as `"$NEXUS_QPDF" --check file.pdf`.
