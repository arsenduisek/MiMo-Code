...
import { ensureNexusGitignore } from "@/config/gitignore"
...
export const getLockFilePath = (dir?: string) => join(dir ?? process.cwd(), ".nexus", ".cron-lock")
...
export const tryAcquireSchedulerLock = (opts?: { dir?: string; lockIdentity?: string }) =>
  Effect.gen(function* () {
    ...
    yield* Effect.tryPromise({
      try: () => ensureNexusGitignore(join(path, "..")),
      catch: () => undefined,
    }).pipe(Effect.orElseSucceed(() => undefined))
