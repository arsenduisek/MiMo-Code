...
//
// Opt-out: `# nofilter` / `# raw` in the command, or env NEXUSCODE_BASH_RAW=1.
//
// Tunables (positive ints via env; defaults shown):
//   NEXUSCODE_EXPERIMENTAL_TOKEN_EFFICIENCY_MAX_LINE_CHARS=500
//   NEXUSCODE_EXPERIMENTAL_TOKEN_EFFICIENCY_LINE_HEAD_KEEP=160
//   NEXUSCODE_EXPERIMENTAL_TOKEN_EFFICIENCY_NEVER_WORSE_MARGIN=0

import { Flag } from "@/flag/flag"

const MAX_LINE_CHARS = Flag.NEXUSCODE_EXPERIMENTAL_TOKEN_EFFICIENCY_MAX_LINE_CHARS
const LINE_HEAD_KEEP = Flag.NEXUSCODE_EXPERIMENTAL_TOKEN_EFFICIENCY_LINE_HEAD_KEEP
const NEVER_WORSE_MARGIN = Flag.NEXUSCODE_EXPERIMENTAL_TOKEN_EFFICIENCY_NEVER_WORSE_MARGIN
...
function shouldSkip(command: string | undefined): boolean {
  if (process.env["NEXUSCODE_BASH_RAW"] === "1") return true
