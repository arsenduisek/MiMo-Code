...
/** Agents that may write memory or the project's `.nexus/` directory. The
 ...
 * memory-only; dream/distill may also write under `<worktree>/.nexus/`.
...
export function assertAgentWriteSandbox(input: {
  ...
  // `<worktree>/.nexus/../src/x.ts` would string-prefix-match `.nexus`
  ...
  const dotDir = path.resolve(input.worktree, ".nexus")
