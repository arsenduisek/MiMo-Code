...
const cli = yargs(args)
  ...
  .middleware(async (opts) => {
    ...
    process.env.AGENT = "1"
    process.env.NEXUSCODE = "1"
    process.env.NEXUSCODE_PID = String(process.pid)

    Log.Default.info("nexus", {
    ...
    const marker = path.join(Global.Path.data, "nexus.db")
    ...
    if (!process.env.NEXUSCODE_DISABLE_CLAUDE_IMPORT && !process.env.NEXUSCODE_CLAUDE_IMPORTED) {
      process.env.NEXUSCODE_CLAUDE_IMPORTED = "1"
      try {
    ...
    if (process.platform === "win32") {
      try {
        ...
        for (const entry of readdirSync(binDir)) {
          if (entry.startsWith("nexus.exe.old_")) {
