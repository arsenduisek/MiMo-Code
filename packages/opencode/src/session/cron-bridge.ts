...
/**
 * Reads NEXUSCODE_DISABLE_CRON from the live environment so a runtime flip stops
 * already-running schedulers (per spec [S10]). NEXUSCODE_EXPERIMENTAL_CRON, by
 * contrast, is read once at start() time.
 */
const isCronDisabled = () => {
  const v = process.env.NEXUSCODE_DISABLE_CRON
  if (!v) return false
...
export interface Interface {
   ...
   * (spec [S8]). No-op when NEXUSCODE_EXPERIMENTAL_CRON is unset.
   */
...
export class CronBridge extends Context.Service<CronBridge, Interface>()("@nexus/CronBridge") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const runKeepaliveSweepFor = (handle: Handle) =>
      Effect.gen(function* () {
        const budget = Flag.NEXUSCODE_LOOP_KEEPALIVE_BUDGET
        const delaySeconds = Flag.NEXUSCODE_LOOP_KEEPALIVE_DELAY_S
