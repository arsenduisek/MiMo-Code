...
export async function run(opts?: { force?: boolean }) {
  ...
  for (const file of files) {
    ...
    try {
      ...
      // user deleted it in nexus, drop the stale mapping and import fresh —
      ...
      Database.transaction((tx) => {
        ...
        if (existing) {
          ...
          // user's nexus-native continuation messages in the same session.
          ...
          // Preserve nexus-owned metadata on re-sync: keep any user rename
