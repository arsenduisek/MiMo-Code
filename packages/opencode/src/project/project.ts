...
async function setupProjectIdEnvironment(workingDir: string): Promise<void> {
  ...
  const localFile = nodePath.join(workingDir, ".nexus-project-id")
  const idFile = nodePath.join(mainGit, "nexus-project-id")
  ...
  // Belt-and-suspenders: ensure .git/info/exclude lists .nexus-project-id
  ...
  if (!existing.includes(".nexus-project-id")) {
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const fakeVcs = Schema.decodeUnknownSync(Schema.optional(ProjectVcs))(Flag.NEXUSCODE_FAKE_VCS)
    ...
    const fromDirectory = Effect.fn("Project.fromDirectory")(function* (directory: string) {
      ...
      const data: DiscoveryResult = yield* Effect.gen(function* () {
        ...
        // turn, which has shipped as a bug for users opening nexus in $HOME.
      ...
      if (Flag.NEXUSCODE_EXPERIMENTAL_ICON_DISCOVERY) yield* discover(existing).pipe(Effect.ignore, Effect.forkIn(scope))
