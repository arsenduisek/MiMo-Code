...
export const USER_AGENT = `nexus/${InstallationChannel}/${InstallationVersion}/${Flag.NEXUSCODE_CLIENT}`
...
// TODO(nexus): uncomment when corresponding channels are supported
...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient | ChildProcessSpawner.ChildProcessSpawner> =
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      // TODO(nexus): uncomment when nexus is published to homebrew
      ...
      const upgradeCurl = Effect.fnUntraced(
        function* (target: string) {
          ...
          const response = yield* httpOk.execute(HttpClientRequest.get(process.env.NEXUSCODE_INSTALL_SCRIPT_URL ?? "https://github.com/arsenduisek/Nexus-Code/install"))
      ...
      const upgradeCurlWindows = Effect.fnUntraced(function* (target: string) {
        ...
        const stageDir = path.join(os.tmpdir(), `nexus_upgrade_${pid}`)

        // Download new version to staging dir (reuses install.ps1 logic)
        const installScriptUrl = process.env.NEXUSCODE_INSTALL_SCRIPT_URL ?? "https://github.com/arsenduisek/Nexus-Code/install.ps1"
        const downloadResult = yield* run(
          ...
          { env: { NEXUSCODE_INSTALL_DIR: stageDir, VERSION: target, INSTALL_SCRIPT_URL: installScriptUrl } },
        )
        if (downloadResult.code !== 0) return downloadResult

        // Replace in-place: Windows allows renaming a running exe
        const stagedExe = path.join(stageDir, "nexus.exe")
      ...
      const methodImpl = Effect.fn("Installation.method")(function* () {
        ...
        const checks: Array<{ name: Method; command: () => Effect.Effect<string> }> = [
          ...
          // TODO(nexus): uncomment when nexus is published to these channels
      ...
      const latestImpl = Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "curl") {
          const base =
            process.env.NEXUS_GITHUB_BASE || "https://api.github.com/repos/arsenduisek/Nexus-Code"
          const json = yield* text([
            "curl",
            "-fsSL",
            "-H",
          ...
          const version = json.match(/"tag_name":\s*"v?([^"]+)"/)?.[1]?.trim() ?? ""
          if (/^\d+\.\d+\.\d+/.test(version)) return version
          return yield* Effect.die(new Error("failed to resolve latest version from GitHub"))
        }
        ...
        // TODO(nexus): uncomment when nexus is published to chocolatey
        ...
        // TODO(nexus): uncomment when nexus is published to scoop
        ...
        // TODO(nexus): uncomment when nexus has github releases
        // const response = yield* httpOk.execute(
      ...
      const upgradeImpl = Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        if (!result || result.code !== 0) {
          // TODO(nexus): restore choco-specific error when choco channel is supported
