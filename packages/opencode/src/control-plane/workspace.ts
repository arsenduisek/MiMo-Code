...
export const create = fn(CreateInput, async (input) => {
  ...
  const env = {
    NEXUSCODE_AUTH_CONTENT: JSON.stringify(await AppRuntime.runPromise(Auth.Service.use((auth) => auth.all()))),
    NEXUSCODE_WORKSPACE_ID: config.id,
    NEXUSCODE_EXPERIMENTAL_WORKSPACES: "true",
...
async function startSync(space: Info) {
  if (!Flag.NEXUSCODE_EXPERIMENTAL_WORKSPACES) return
