...
export const dict = {
  ...
  "tui.tips.free_api_sunset":
    "無料 API サービスは終了しました。{highlight}/login{/highlight} でログインしてください。Nexus Token Plan を購読するか、サードパーティ API を設定して Nexus Code をご利用ください。",
  ...
  "tui.tips.config_files":
    "サーバ設定は {highlight}nexus.json{/highlight}、TUI 設定は {highlight}tui.json{/highlight} を作成します",
  "tui.tips.global_config":
    "TUI 設定をグローバルに置くなら {highlight}~/.config/nexus/tui.json{/highlight} を使います",
  ...
  "tui.tips.custom_command":
    "{highlight}.nexus/command/{/highlight} に {highlight}.md{/highlight} ファイルを追加すると再利用可能なカスタムプロンプトを定義できます",
  ...
  "tui.tips.custom_agent":
    "{highlight}.nexus/agent/{/highlight} に {highlight}.md{/highlight} ファイルを追加して専用 AI ペルソナを作れます",
  ...
  "tui.tips.custom_tool":
    "{highlight}.nexus/tools/{/highlight} に {highlight}.ts{/highlight} ファイルを作って新しい LLM ツールを定義します",
  "tui.tips.tool_scripts": "ツール定義から Python、Go などのスクリプトを呼び出せます",
  "tui.tips.plugins":
    "{highlight}.nexus/plugin/{/highlight} に {highlight}.ts{/highlight} ファイルを追加してイベントフックを実装できます",
  ...
  "tui.tips.theme_files": "{highlight}.nexus/themes/{/highlight} ディレクトリに JSON テーマファイルを作成します",
  ...
  "tui.skill.nexus.description": "NexusCode の機能・設定・コマンドに関するセルフドキュメント",
  ...
  "tui.skill.drive-nexus.description": "別の NexusCode プロセスをプログラムで操作——ヘッドレス JSON イベントまたは tmux 経由のインタラクティブ TUI",
  ...
  "tui.dialog.free_api_sunset.message":
    "/login でログインしてください。Nexus Token Plan を購読するか、サードパーティ API を設定して Nexus Code をご利用ください。",
  ...
  "tui.model.nexus_auto.name": "Nexus Auto（Nexus-V2.5 7月26日 18:00 まで無料 · UTC+8）",
  ...
  "tui.dialog.try_best.handoff.description": "未完了の作業をこのハーネスへ委任するよう Nexus に依頼",
  ...
  "tui.voice.error.no_auth": "/connect で Nexus アカウントに接続するか、voice.asr_model で別のプロバイダーを設定してください",
  ...
  "skip_permissions.body":
    "--dangerously-skip-permissions を指定して起動しました。Nexus Code は承認を求めずにファイルの読み取り・編集・実行やシェルコマンドの実行を行います。設定で明示的に拒否（deny）したルールのみが引き続き適用されます。発生したいかなる結果についてもあなた自身が全責任を負います。",
