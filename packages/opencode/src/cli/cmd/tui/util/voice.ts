...
const DEFAULT_ASR_MODEL = "gemini/gemini-2.5-flash"
const DEFAULT_CONTROL_MODEL = "gemini/gemini-2.5-flash"
...
export function resolveCredentials(
  ...
  const baseUrl = (provider.options?.baseURL as string)
    ...
    || (config.providerID === "gemini" ? "https://generativelanguage.googleapis.com/v1beta/openai" : undefined)
...
function parseModelID(modelID: string): VoiceProviderConfig {
  ...
  if (slashIndex < 1) return { providerID: "gemini", model: modelID }
...
// Nexus ASR uses a proprietary data-URL audio format and asr_options field, not the standard OpenAI input_audio schema.
