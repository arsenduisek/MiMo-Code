...
async function load(input: { api: Api; config: TuiConfig.Info }) {
  ...
  try {
    await Instance.provide({
      directory: cwd,
      fn: async () => {
        const records = Flag.NEXUSCODE_PURE ? [] : (config.plugin_origins ?? [])
        if (Flag.NEXUSCODE_PURE && config.plugin_origins?.length) {
        ...
        // File-based TUI plugins from .nexus/tui/*.{ts,tsx}
        const configDirs = [
          path.join(Global.Path.config, "tui"),
          path.join(cwd, ".nexus", "tui"),
        ]
