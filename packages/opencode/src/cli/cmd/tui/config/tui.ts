...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // folders between cwd and home, and NEXUSCODE_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.NEXUSCODE_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit NEXUSCODE_TUI_CONFIG override, if set.
  if (Flag.NEXUSCODE_TUI_CONFIG) {
    const configFile = Flag.NEXUSCODE_TUI_CONFIG
  ...
  // 4. `.nexus` directories (and NEXUSCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".nexus") || dir === Flag.NEXUSCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".nexus") && dir !== Flag.NEXUSCODE_CONFIG_DIR) continue
...
const TUI_SCHEMA_URL = "https://github.com/arsenduisek/Nexus-Code/docs/tui.json"
