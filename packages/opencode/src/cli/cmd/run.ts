...
export const RunCommand = cmd({
  ...
  builder: (yargs: Argv) => {
    return yargs
      ...
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to NEXUSCODE_SERVER_PASSWORD)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function share(sdk: OpencodeClient, sessionID: string) {
      ...
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.NEXUSCODE_AUTO_SHARE && !args.share) return
    ...
    if (args.attach) {
      const headers = (() => {
        const password = args.password ?? process.env.NEXUSCODE_SERVER_PASSWORD
        if (!password) return undefined
        const username = process.env.NEXUSCODE_SERVER_USERNAME ?? "nexus"
