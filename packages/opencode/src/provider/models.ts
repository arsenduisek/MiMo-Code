...
function url() {
  return Flag.NEXUSCODE_MODELS_URL || "https://models.dev"
}
...
export const Data = lazy(async () => {
  const result = await Filesystem.readJson(Flag.NEXUSCODE_MODELS_PATH ?? filepath).catch(() => {})
  ...
  if (Flag.NEXUSCODE_DISABLE_MODELS_FETCH) return {}
...
if (!Flag.NEXUSCODE_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
  void refresh()
  setInterval(
    async () => {
      await refresh()
