...
// checkout (at ../../nexusapi/packages/opencode/src/ext), it is copied into
...
const overlaySrc = path.resolve(pkgDir, "../../nexusapi/packages/opencode/src/ext")
...
const proc = Bun.spawn(["bun", "run", "--conditions=browser", "src/index.ts", ...process.argv.slice(2)], {
  ...
  env: { ...process.env, NEXUSCODE_HOME: process.env.NEXUSCODE_HOME ?? path.resolve(pkgDir, "../../.dev-home") },
})
