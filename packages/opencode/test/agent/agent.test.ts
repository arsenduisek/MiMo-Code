...
test("plan denies edits except plan files (via runtimePermission)", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(Permission.evaluate("edit", ".nexus/plans/foo.md", rt).action).toBe("allow")
    },
  })
})
...
test("skill directories are allowed for external_directory", async () => {
  await using tmp = await tmpdir({
    git: true,
    init: async (dir) => {
      const skillDir = path.join(dir, ".nexus", "skill", "perm-skill")
      await Bun.write(
...
`,
  ...
  try {
    await Instance.provide({
      ...
      fn: async () => {
        const build = await load(tmp.path, (svc) => svc.get("build"))
        const skillDir = path.join(tmp.path, ".nexus", "skill", "perm-skill")
