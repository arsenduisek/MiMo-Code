...
describe("postinstall", () => {
  ...
  test("creates .nexus binary cache from platform package", async () => {
    ...
    const cached = path.join(binDir, ".nexus")
  ...
  test("prints migration notice to stdout", async () => {
    ...
    expect(stdout).toContain("Recommended: install NexusCode natively for a better")
  ...
  test("exits with error when binary package is missing", async () => {
    ...
    expect(result.stdout.toString()).toContain("Recommended: install NexusCode natively")
  })
