...
test("getCronFilePath joins .nexus/scheduled_tasks.json", () => {
  expect(getCronFilePath("/tmp/x").endsWith("/.nexus/scheduled_tasks.json")).toBe(true)
})
...
test("malformed cron in file is silently dropped", async () => {
  ...
  await fs.mkdir(join(dir, ".nexus"), { recursive: true })
  await fs.writeFile(
    getCronFilePath(dir),
    JSON.stringify({
      tasks: [
