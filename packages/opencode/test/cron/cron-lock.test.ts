...
test("acquire returns true on fresh dir and writes lock file", async () => {
  ...
  expect(existsSync(join(dir, ".nexus", ".cron-lock"))).toBe(true)
  cleanup(dir)
})
...
test("creates .gitignore alongside lock file in fresh directory", async () => {
  const dir = fresh()
  expect(await run(tryAcquireSchedulerLock({ dir }))).toBe(true)
  const gitignore = join(dir, ".nexus", ".gitignore")
...
test("acquire takes over when previous owner is dead (ESRCH)", async () => {
  const dir = fresh()
  mkdirSync(join(dir, ".nexus"), { recursive: true })
  writeFileSync(join(dir, ".nexus", ".cron-lock"), JSON.stringify({ pid: 999_999, startedAt: 0 }))
  expect(await run(tryAcquireSchedulerLock({ dir }))).toBe(true)
  cleanup(dir)
})
...
test("acquire overwrites malformed lock file", async () => {
  const dir = fresh()
  mkdirSync(join(dir, ".nexus"), { recursive: true })
  writeFileSync(join(dir, ".nexus", ".cron-lock"), "garbage{not json")
  expect(await run(tryAcquireSchedulerLock({ dir }))).toBe(true)
  cleanup(dir)
})
...
test("release removes our own lock", async () => {
  ...
  expect(existsSync(join(dir, ".nexus", ".cron-lock"))).toBe(false)
  cleanup(dir)
})
