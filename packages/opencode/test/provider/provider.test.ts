...
// smart default picks a vision-capable model with in-house (nexus/gemini)
...
test("getVisionModel picks cheapest vision model", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(
        path.join(dir, "nexus.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
          provider: VISION_PROVIDER,
          enabled_providers: ["nexus", "vendor"],
        }),
      )
    },
  })
  await Instance.provide({
    directory: tmp.path,
    fn: async () => {
      // vendor-cheap-vision (cost 1) beats nexus-vision (cost 100);
...
test("plugin config providers persist after instance dispose", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const root = path.join(dir, ".nexus", "plugin")
