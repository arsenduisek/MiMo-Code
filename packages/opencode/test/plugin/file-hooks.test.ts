...
const disableDefault = process.env.NEXUSCODE_DISABLE_DEFAULT_PLUGINS
process.env.NEXUSCODE_DISABLE_DEFAULT_PLUGINS = "1"
...
afterAll(() => {
  if (disableDefault === undefined) {
    delete process.env.NEXUSCODE_DISABLE_DEFAULT_PLUGINS
    return
  }
  process.env.NEXUSCODE_DISABLE_DEFAULT_PLUGINS = disableDefault
})
...
describe("plugin file hooks", () => {
  test("loads hooks from .nexus/hooks and picks up external edits without reload call", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Bun.write(path.join(dir, ".nexus", "hooks", "greet.ts"), hookSource("v1"))
        await Bun.write(path.join(dir, "nexus.json"), '{}')
      },
    })
    const hookFile = path.join(tmp.path, ".nexus", "hooks", "greet.ts")
  ...
  test("detects newly added hook files", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await fs.promises.mkdir(path.join(dir, ".nexus", "hooks"), { recursive: true })
        await Bun.write(path.join(dir, "nexus.json"), '{}')
      },
    })
    ...
    const out = await Instance.provide({
      ...
      fn: async () =>
        Effect.gen(function* () {
          ...
          fs.writeFileSync(path.join(tmp.path, ".nexus", "hooks", "late.ts"), hookSource("late"))
          yield* Effect.promise(() => Bun.sleep(1200))
  ...
  test("dispatches bus events to file hook event handlers", async () => {
    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        ...
        await Bun.write(path.join(dir, "nexus.json"), '{}')
        await Bun.write(
          path.join(dir, ".nexus", "hooks", "listener.ts"),
          [
            "import fs from 'fs'",
            "export default {",
            "  event: async ({ event }) => {",
