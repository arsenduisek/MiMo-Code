...
export function withTmpdirOutsideGit<T>(fn: () => Promise<T>): Promise<T> {
  const prev = process.env["NEXUSCODE_TEST_TMPDIR_ROOT"]
  process.env["NEXUSCODE_TEST_TMPDIR_ROOT"] = outsideGitTmpRoot()
  return fn().finally(() => {
    if (prev !== undefined) process.env["NEXUSCODE_TEST_TMPDIR_ROOT"] = prev
    else delete process.env["NEXUSCODE_TEST_TMPDIR_ROOT"]
  })
}
...
export async function tmpdir<T>(options?: TmpDirOptions<T>) {
  const prevRoot = options?.outsideGit ? process.env["NEXUSCODE_TEST_TMPDIR_ROOT"] : undefined
  if (options?.outsideGit) process.env["NEXUSCODE_TEST_TMPDIR_ROOT"] = outsideGitTmpRoot()
  const dirpath = sanitizePath(
    path.join(process.env["NEXUSCODE_TEST_TMPDIR_ROOT"] ?? os.tmpdir(), "nexus-test-" + Math.random().toString(36).slice(2)),
  )
    ...
    [Symbol.asyncDispose]: async () => {
      try {
        ...
        if (options?.outsideGit) {
          if (prevRoot !== undefined) process.env["NEXUSCODE_TEST_TMPDIR_ROOT"] = prevRoot
          else delete process.env["NEXUSCODE_TEST_TMPDIR_ROOT"]
        }
      }
    },
