...
// patterns like `.nexus/` in root .gitignore affect test results.
...
describe("file.ripgrep", () => {
    ...
    withTmpdirOutsideGit(async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          await Bun.write(path.join(dir, ".nexus", "thing.json"), "{}")
        },
      })
      ...
      expect(files.includes(path.join(".nexus", "thing.json"))).toBe(true)
    }))
  ...
  test("hidden false excludes hidden", async () => {
    ...
    expect(files.includes("visible.txt")).toBe(true)
    expect(files.includes(path.join(".nexus", "thing.json"))).toBe(false)
  })
