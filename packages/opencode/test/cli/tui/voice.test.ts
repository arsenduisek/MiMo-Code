...
describe("voice", () => {
  describe("resolveVoiceConfig", () => {
    test("returns gemini defaults when no config provided", async () => {
      const { resolveVoiceConfig } = await import("../../../src/cli/cmd/tui/util/voice")
      const result = resolveVoiceConfig(undefined)
      expect(result.asr.providerID).toBe("gemini")
      expect(result.asr.model).toBe("nexus-v2.5-asr")
      expect(result.control.providerID).toBe("gemini")
      expect(result.control.model).toBe("nexus-v2.5")
    })

    test("returns gemini defaults when config is empty object", async () => {
      const { resolveVoiceConfig } = await import("../../../src/cli/cmd/tui/util/voice")
    ...
    test("parses custom asr_model correctly", async () => {
      const { resolveVoiceConfig } = await import("../../../src/cli/cmd/tui/util/voice")
      const result = resolveVoiceConfig({ asr_model: "newapi/nexus-v2.5-asr" })
    ...
    test("parses custom control_model correctly", async () => {
      const { resolveVoiceConfig } = await import("../../../src/cli/cmd/tui/util/voice")
      const result = resolveVoiceConfig({ control_model: "openrouter/gemini/nexus-v2.5" })
      expect(result.asr.providerID).toBe("gemini")
      expect(result.asr.model).toBe("nexus-v2.5-asr")
      expect(result.control.providerID).toBe("openrouter")
      expect(result.control.model).toBe("gemini/nexus-v2.5")
    })

    test("supports both custom asr and control", async () => {
      const { resolveVoiceConfig } = await import("../../../src/cli/cmd/tui/util/voice")
      const result = resolveVoiceConfig({
        asr_model: "newapi/nexus-v2.5-asr",
    ...
    test("treats no-slash model ID as model with default provider", async () => {
      const { resolveVoiceConfig } = await import("../../../src/cli/cmd/tui/util/voice")
      const result = resolveVoiceConfig({ asr_model: "nexus-v2.5-asr" })
      expect(result.asr.providerID).toBe("gemini")
      expect(result.asr.model).toBe("nexus-v2.5-asr")
    })
  })
  ...
  describe("resolveCredentials", () => {
    ...
    test("resolves credentials from provider.key and options.baseURL", async () => {
      ...
      const result = resolveCredentials(
        ...
        { providerID: "openrouter", model: "gemini/nexus-v2.5" },
      )
    ...
    test("resolves apiKey from options.apiKey when provider.key is absent", async () => {
      ...
      const result = resolveCredentials(
        ...
        { providerID: "internal", model: "nexus-v2.5" },
      )
    ...
    test("returns no_url for non-gemini provider without baseURL", async () => {
      const { resolveCredentials } = await import("../../../src/cli/cmd/tui/util/voice")
      const result = resolveCredentials(
    ...
    test("falls back to hardcoded URL only for gemini", async () => {
      const { resolveCredentials } = await import("../../../src/cli/cmd/tui/util/voice")
      const result = resolveCredentials(
        [makeProvider("gemini", { key: "sk-x" })],
        { providerID: "gemini", model: "nexus-v2.5-asr" },
      )
      expect(result).toEqual({ apiKey: "sk-x", baseUrl: "https://generativelanguage.googleapis.com/v1" })
    })
  })
  ...
  describe("processVoiceControl", () => {
    ...
    test("returns null on network error with custom model", async () => {
      ...
      const result = await processVoiceControl({
        ...
        apiKey: "test-key",
        baseUrl: "http://127.0.0.1:1",
        model: "custom-provider/nexus-v2.5",
        currentText: "hello",
        currentAgent: "build",
        availableAgents: ["build", "plan"],
        sendEnabled: false,
