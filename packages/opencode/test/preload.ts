...
const fixtureRoot = path.join(process.cwd(), ".nexus-test-fixtures-" + process.pid)
await fs.mkdir(fixtureRoot, { recursive: true })
process.env["NEXUSCODE_TEST_TMPDIR_ROOT"] = fixtureRoot
afterAll(async () => {
...
process.env["NEXUSCODE_MODELS_PATH"] = path.join(import.meta.dir, "tool", "fixtures", "models-api.json")
...
process.env["NEXUSCODE_TEST_MANAGED_CONFIG_DIR"] = testManagedConfigDir
process.env["NEXUSCODE_DISABLE_DEFAULT_PLUGINS"] = "true"
...
delete process.env["NEXUSCODE_SERVER_PASSWORD"]
delete process.env["NEXUSCODE_SERVER_USERNAME"]
delete process.env["NEXUSCODE_HOME"]

// Use in-memory sqlite
process.env["NEXUSCODE_DB"] = ":memory:"
...
// NEXUSCODE_EXPERIMENTAL_ORCHESTRATOR; the orchestrator test suites exercise the
// feature, so enable it here (Flag is read once at import — must be set first).
process.env["NEXUSCODE_EXPERIMENTAL_ORCHESTRATOR"] = "true"
