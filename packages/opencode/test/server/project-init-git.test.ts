...
// inherit a parent .git. We temporarily set Flag.NEXUSCODE_SERVER_PASSWORD to
// bypass the middleware cwd containment check and include auth headers.
const TEST_PASSWORD = "init-git-test"
const authHeader = `Basic ${Buffer.from(`nexus:${TEST_PASSWORD}`).toString("base64")}`

describe("project.initGit endpoint", () => {
  test("initializes git and reloads immediately", async () => {
    const prevFlag = (Flag as any).NEXUSCODE_SERVER_PASSWORD
    ;(Flag as any).NEXUSCODE_SERVER_PASSWORD = TEST_PASSWORD
    try {
      ...
      try {
        ...
        expect(await Filesystem.exists(path.join(tmp.path, ".git", "nexus"))).toBe(false)
      ...
      ;(Flag as any).NEXUSCODE_SERVER_PASSWORD = prevFlag
    }
  })
