  ...
  () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ dir, llm }) {
        const prev = process.env["NEXUSCODE_PROMPT_HEARTBEAT_INTERVAL_MS"]
        process.env["NEXUSCODE_PROMPT_HEARTBEAT_INTERVAL_MS"] = "50"
        yield* Effect.addFinalizer(() =>
          Effect.sync(() => {
            if (prev === undefined) delete process.env["NEXUSCODE_PROMPT_HEARTBEAT_INTERVAL_MS"]
            else process.env["NEXUSCODE_PROMPT_HEARTBEAT_INTERVAL_MS"] = prev
          }),
        )
