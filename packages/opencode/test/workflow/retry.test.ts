...
afterEach(async () => {
  delete process.env.NEXUSCODE_TEST_SPAWN_FAIL_ONCE
...
// completed run. The NEXUSCODE_TEST_SPAWN_FAIL_ONCE seam forces the first N shared
...
describe("WorkflowRuntime agent() retry", () => {
  it.live("retries a spawn-reject and succeeds on the second attempt", () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        process.env.NEXUSCODE_TEST_SPAWN_FAIL_ONCE = "1" // first spawn attempt throws
  ...
  it.live("no retry option => a spawn-reject is not retried (one failed attempt, run returns null)", () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* ({ llm }) {
        process.env.NEXUSCODE_TEST_SPAWN_FAIL_ONCE = "1"
        const runtime = yield* WorkflowRuntime.Service
        const session = yield* Session.Service
        const bus = yield* Bus.Service
        const failed: string[] = []
  ...
  it.live("retry exhausted => still null; every attempt emits a failed event", () =>
    provideTmpdirServer(
      Effect.fnUntraced(function* () {
        process.env.NEXUSCODE_TEST_SPAWN_FAIL_ONCE = "5" // more than attempts -> all fail
        const runtime = yield* WorkflowRuntime.Service
        const session = yield* Session.Service
        const bus = yield* Bus.Service
        const failed: string[] = []
