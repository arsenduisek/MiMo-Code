...
async function tmpdir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "nexus-home-test-"))
  return {
    path: dir,
...
describe("NEXUSCODE_HOME end-to-end", () => {
  test("sets all four paths under the profile root", async () => {
    await using tmp = await tmpdir()
    const result = await runWorker({ NEXUSCODE_HOME: tmp.path })
  ...
  test("auto-creates all four subdirs under a nonexistent root", async () => {
    ...
    const result = await runWorker({ NEXUSCODE_HOME: nested })
    expect(result.ok).toBe(true)
    if (!result.ok) return
  ...
  test("two profiles have fully isolated data (write in A, not visible in B, A survives restart)", async () => {
    ...
    const resultA = await runWorker({ NEXUSCODE_HOME: a.path })
    ...
    const resultB = await runWorker({ NEXUSCODE_HOME: b.path })
    ...
    const resultA2 = await runWorker({ NEXUSCODE_HOME: a.path })
  ...
  test("relative path causes startup failure with clear error", async () => {
    const result = await runWorker({ NEXUSCODE_HOME: "./relative-path" })
    expect(result.ok).toBe(false)
    if (result.ok) return
    expect(result.stderr).toMatch(/NEXUSCODE_HOME must be an absolute path/)
  })
  ...
  test("empty string falls through to XDG behavior", async () => {
    ...
    expect(result.ok).toBe(true)
    if (!result.ok) return
    // Paths should reflect XDG layout (ends with "/nexus"), not NEXUSCODE_HOME layout
    expect(result.paths.config).toBe(path.join(tmp.path, "config", "nexus"))
    expect(result.paths.data).toBe(path.join(tmp.path, "data", "nexus"))
  })
})
