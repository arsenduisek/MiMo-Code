...
const originalCronFlag = Flag.NEXUSCODE_EXPERIMENTAL_CRON
afterEach(async () => {
  ;(Flag as { NEXUSCODE_EXPERIMENTAL_CRON: boolean }).NEXUSCODE_EXPERIMENTAL_CRON = originalCronFlag
...
beforeEach(() => {
  ...
  delete process.env.NEXUSCODE_LOOP_KEEPALIVE_BUDGET
  delete process.env.NEXUSCODE_LOOP_KEEPALIVE_DELAY_S
  ;(Flag as { NEXUSCODE_EXPERIMENTAL_CRON: boolean }).NEXUSCODE_EXPERIMENTAL_CRON = true
})
...
describe("cron-bridge keepalive sweep", () => {
  ...
  it.live("budget=0 ends loop immediately on first turn without a re-arm", () =>
    Effect.gen(function* () {
      process.env.NEXUSCODE_LOOP_KEEPALIVE_BUDGET = "0"
