...
beforeEach(() => {
  ...
  delete process.env.NEXUSCODE_DISABLE_CRON
  process.env.NEXUSCODE_EXPERIMENTAL_CRON = "1"
})
  ...
  try {
    await harness(captured, ({ bridge, scheduler }) =>
      Effect.gen(function* () {
        ...
        // isKilled honors process.env.NEXUSCODE_DISABLE_CRON live (verified by
        // forcing it and observing armLoop refuse to schedule).
        process.env.NEXUSCODE_DISABLE_CRON = "1"
...
test("cron-bridge is a no-op when NEXUSCODE_EXPERIMENTAL_CRON is explicitly disabled", async () => {
  const captured: { value: CapturedPrompt[] } = { value: [] }
  const originalFlag = Flag.NEXUSCODE_EXPERIMENTAL_CRON
  ;(Flag as { NEXUSCODE_EXPERIMENTAL_CRON: boolean }).NEXUSCODE_EXPERIMENTAL_CRON = false
  const dir = freshDir()
  try {
    await harness(captured, ({ bridge, scheduler }) =>
      Effect.gen(function* () {
    ...
    ;(Flag as { NEXUSCODE_EXPERIMENTAL_CRON: boolean }).NEXUSCODE_EXPERIMENTAL_CRON = originalFlag
    rmSync(dir, { recursive: true, force: true })
  }
})
...
test("cron-bridge resets sentinel cache on main-agent Compacted, ignores subagent slice", async () => {
  ...
  try {
    ...
    mkdirSync2(join(wsDir, ".nexus"), { recursive: true })
    writeFileSync2(join(wsDir, ".nexus", "loop.md"), "cached body")
