...
describe("util.env-info", () => {
  test("returns host, user, runtime, and nexus metadata", async () => {
    ...
    expect(info.nexus.version).toBeTruthy()
    expect(info.nexus.channel).toBeTruthy()
    expect(info.nexus.installation_id).toMatch(
