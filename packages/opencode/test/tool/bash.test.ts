...
async function isolateZshDotfiles() {
  ...
  const zdotdir = path.join(os.tmpdir(), `nexus-zdotdir-${Math.random().toString(36).slice(2)}`)
    ...
    try {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // The tmpdir git fixture sets user.name=Test / user.email=test@nexus.test.
          expect(result.metadata.output).toContain("GIT_AUTHOR_NAME=Test")
          expect(result.metadata.output).toContain("GIT_AUTHOR_EMAIL=test@nexus.test")
          expect(result.metadata.output).toContain("GIT_COMMITTER_NAME=Test")
          expect(result.metadata.output).toContain("GIT_COMMITTER_EMAIL=test@nexus.test")
        },
      })
    } finally {
      restoreEnv(saved)
    ...
    try {
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // user.name=Test / user.email=test@nexus.test.
          expect(result.metadata.output).toContain("GIT_AUTHOR_EMAIL=test@nexus.test")
          expect(result.metadata.output).toContain("GIT_COMMITTER_NAME=Test")
          expect(result.metadata.output).toContain("GIT_COMMITTER_EMAIL=test@nexus.test")
        },
  ...
  if (process.platform === "win32") {
    ...
    for (const item of ps) {
      test(
        ...
        withShell(item, async () => {
          const key = "NEXUSCODE_TEST_MISSING"
