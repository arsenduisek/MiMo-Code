...
describe("assertAgentWriteSandbox", () => {
  ...
  test("checkpoint-writer may NOT write under <worktree>/.nexus", () => {
    expect(() =>
      assertAgentWriteSandbox({
        target: path.join(WORKTREE, ".nexus", "skills", "unsafe", "SKILL.md"),
        agentName: "checkpoint-writer",
        memoryRoot: MEMORY_ROOT,
        worktree: WORKTREE,
      }),
  ...
  for (const agent of ["dream", "distill"] as const) {
    ...
    test(`${agent} may write under <worktree>/.nexus`, () => {
      expect(() =>
        assertAgentWriteSandbox({
          target: path.join(WORKTREE, ".nexus", "skills", "foo", "SKILL.md"),
          agentName: agent,
          memoryRoot: MEMORY_ROOT,
          worktree: WORKTREE,
        }),
    ...
    test(`${agent} may NOT escape .nexus via .. into a source file`, () => {
      expect(() =>
        assertAgentWriteSandbox({
          target: path.join(WORKTREE, ".nexus", "..", "src", "index.ts"),
          agentName: agent,
          memoryRoot: MEMORY_ROOT,
          worktree: WORKTREE,
        }),
    ...
    test(`${agent} unnormalized-but-in-bounds path still passes`, () => {
      expect(() =>
        assertAgentWriteSandbox({
          target: path.join(WORKTREE, ".nexus", "skills", "..", "agent", "x.md"),
          agentName: agent,
          memoryRoot: MEMORY_ROOT,
          worktree: WORKTREE,
        }),
