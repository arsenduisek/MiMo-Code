...
describe("tool.registry", () => {
  it.live("loads tools from .nexus/tool (singular)", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        const opencode = path.join(dir, ".nexus")
  ...
  it.live("loads tools from .nexus/tools (plural)", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        const opencode = path.join(dir, ".nexus")
  ...
  it.live("keeps the reserved MCP search tool when a custom tool conflicts", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        const tools = path.join(dir, ".nexus", "tools")
        yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
        yield* Effect.promise(() =>
          Bun.write(
