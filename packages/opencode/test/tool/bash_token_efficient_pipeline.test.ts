...
describe("createPipeline", () => {
  ...
  test("NEXUSCODE_BASH_RAW=1 bypasses the pipeline", () => {
    const prev = process.env.NEXUSCODE_BASH_RAW
    process.env.NEXUSCODE_BASH_RAW = "1"
    try {
      const drop: CleanPlugin = { name: "drop", apply: () => "" }
      const result = createPipeline([drop]).run("payload", { command: "ls" })
      expect(result.text).toBe("payload")
    } finally {
      if (prev === undefined) delete process.env.NEXUSCODE_BASH_RAW
      else process.env.NEXUSCODE_BASH_RAW = prev
    }
  })
