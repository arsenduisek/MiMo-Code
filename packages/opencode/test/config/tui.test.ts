...
afterEach(async () => {
  delete process.env.NEXUSCODE_CONFIG
  delete process.env.NEXUSCODE_TUI_CONFIG
  await fs.rm(path.join(Global.Path.config, "nexus.json"), { force: true }).catch(() => {})
  await fs.rm(path.join(Global.Path.config, "nexus.jsonc"), { force: true }).catch(() => {})
...
test("keeps server and tui plugin merge semantics aligned", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const local = path.join(dir, ".nexus")
      await fs.mkdir(local, { recursive: true })

      await Bun.write(
        path.join(Global.Path.config, "nexus.json"),
        JSON.stringify(
          {
...
test("migrates tui-specific keys from nexus.json when tui.json does not exist", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(
  ...
  const server = JSON.parse(await Filesystem.readText(path.join(tmp.path, "nexus.json")))
  expect(server.theme).toBeUndefined()
  expect(server.keybinds).toBeUndefined()
  expect(server.tui).toBeUndefined()
  expect(await Filesystem.exists(path.join(tmp.path, "nexus.json.tui-migration.bak"))).toBe(true)
...
test("skips migration when nexus.jsonc is syntactically invalid", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "nexus.jsonc.tui-migration.bak"))).toBe(false)
  const source = await Filesystem.readText(path.join(tmp.path, "nexus.jsonc"))
...
test("skips migration when tui.json already exists", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(path.join(dir, "nexus.json"), JSON.stringify({ theme: "legacy" }, null, 2))
  ...
  expect(await Filesystem.exists(path.join(tmp.path, "nexus.json.tui-migration.bak"))).toBe(false)
})
...
test.skip("continues loading tui config when legacy source cannot be stripped", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await Bun.write(path.join(dir, "nexus.json"), JSON.stringify({ theme: "readonly-theme" }, null, 2))
    },
  })

  const source = path.join(tmp.path, "nexus.json")
...
test("migration backup preserves JSONC comments", async () => {
  ...
  const backup = await Filesystem.readText(path.join(tmp.path, "nexus.jsonc.tui-migration.bak"))
...
test("migrates legacy tui keys across multiple nexus.json levels", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const nested = path.join(dir, "apps", "client")
      await fs.mkdir(nested, { recursive: true })
      await Bun.write(path.join(dir, "nexus.json"), JSON.stringify({ theme: "root-theme" }, null, 2))
      await Bun.write(path.join(nested, "nexus.json"), JSON.stringify({ theme: "nested-theme" }, null, 2))
    },
  })
...
test("project config takes precedence over NEXUSCODE_TUI_CONFIG (matches NEXUSCODE_CONFIG)", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      ...
      process.env.NEXUSCODE_TUI_CONFIG = custom
    },
  })
...
test("NEXUSCODE_TUI_CONFIG provides settings when no project config exists", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const custom = path.join(dir, "custom-tui.json")
...
test("does not derive tui path from NEXUSCODE_CONFIG", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      const customDir = path.join(dir, "custom")
      await fs.mkdir(customDir, { recursive: true })
      await Bun.write(path.join(customDir, "nexus.json"), JSON.stringify({ model: "test/model" }))
      await Bun.write(path.join(customDir, "tui.json"), JSON.stringify({ theme: "should-not-load" }))
      process.env.NEXUSCODE_CONFIG = path.join(customDir, "nexus.json")
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.theme).toBeUndefined()
...
test("loads .nexus/tui.json", async () => {
  await using tmp = await tmpdir({ outsideGit: true,
    init: async (dir) => {
      await fs.mkdir(path.join(dir, ".nexus"), { recursive: true })
      await Bun.write(path.join(dir, ".nexus", "tui.json"), JSON.stringify({ diff_style: "stacked" }, null, 2))
    },
  })
  const config = await getTuiConfig(tmp.path)
  expect(config.diff_style).toBe("stacked")
...
test("injects $schema when missing from tui.json", async () => {
  ...
  await getTuiConfig(tmp.path)
  const text = await Filesystem.readText(path.join(tmp.path, "tui.json"))
  expect(text).toContain('"$schema": "https://github.com/arsenduisek/Nexus-Code/docs/tui.json"')
...
test("does not modify $schema when pointing to a custom URL", async () => {
  ...
  expect(text).not.toContain("github.com/arsenduisek/Nexus-Code")
})
