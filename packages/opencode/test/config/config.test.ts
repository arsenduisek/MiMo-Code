...
const managedConfigDir = process.env.NEXUSCODE_TEST_MANAGED_CONFIG_DIR!
...
async function writeManagedSettings(settings: object, filename = "nexus.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "nexus.json"))
...
test("migrates old opencode.ai $schema URL to github.com/arsenduisek/Nexus-Code", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
        path.join(dir, "nexus.json"),
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await load()
      const content = await Filesystem.readText(path.join(tmp.path, "nexus.json"))
      expect(content).toContain('"$schema": "https://github.com/arsenduisek/Nexus-Code/docs/config.json"')
...
test("does not modify custom $schema URL", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      expect(content).not.toContain("github.com/arsenduisek/Nexus-Code")
    },
  })
})
...
test("preserves JSONC comments when injecting $schema", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      await load()
      const content = await Filesystem.readText(path.join(tmp.path, "nexus.jsonc"))
      expect(content).toContain("$schema")
...
test("resolves env templates in account config with account token", async () => {
  const originalControlToken = process.env["NEXUSCODE_CONSOLE_TOKEN"]
  ...
  const fakeAccount = Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:NEXUSCODE_CONSOLE_TOKEN}" } } },
        }),
      ),
  ...
  try {
    ...
    if (originalControlToken !== undefined) {
      process.env["NEXUSCODE_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["NEXUSCODE_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "nexus.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .nexus directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".nexus")
...
test("loads agents from .nexus/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".nexus")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .nexus/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".nexus")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("loads commands from .nexus/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".nexus")
      await fs.mkdir(opencodeDir, { recursive: true })
...
test("nexus command overrides .claude command on name collision", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(
...
from claude`,
      ...
      await Filesystem.write(
        path.join(dir, ".nexus", "command", "dup.md"),
        `---
...
test("does not try to install dependencies in read-only NEXUSCODE_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.NEXUSCODE_CONFIG_DIR
  process.env.NEXUSCODE_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.NEXUSCODE_CONFIG_DIR
    else process.env.NEXUSCODE_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable NEXUSCODE_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .nexus config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".nexus")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .nexus config with different plugins
      await Filesystem.write(
        path.join(opencodeDir, "nexus.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .nexus config with some overlapping plugins
      await Filesystem.write(
        path.join(opencodeDir, "nexus.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".nexus")
...
// Note: preload.ts sets NEXUSCODE_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .nexus config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .nexus directory config enables it
      const opencodeDir = path.join(dir, ".nexus")
      await fs.mkdir(opencodeDir, { recursive: true })
      await Filesystem.write(
        path.join(opencodeDir, "nexus.json"),
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.nexus/plugin/oh-my-opencode.js"]
...
describe("NEXUSCODE_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["NEXUSCODE_DISABLE_PROJECT_CONFIG"]
    process.env["NEXUSCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["NEXUSCODE_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["NEXUSCODE_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .nexus/ directories when flag is set", async () => {
    const originalEnv = process.env["NEXUSCODE_DISABLE_PROJECT_CONFIG"]
    process.env["NEXUSCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .nexus directory with a command
          const opencodeDir = path.join(dir, ".nexus", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          // Project .nexus should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["NEXUSCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NEXUSCODE_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["NEXUSCODE_CONFIG_DIR"]
      process.env["NEXUSCODE_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["NEXUSCODE_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["NEXUSCODE_CONFIG_DIR"]
      } else {
        process.env["NEXUSCODE_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("NEXUSCODE_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["NEXUSCODE_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["NEXUSCODE_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["NEXUSCODE_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["NEXUSCODE_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await load()
          // Should load from NEXUSCODE_CONFIG_DIR, not project
...
describe("NEXUSCODE_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in NEXUSCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["NEXUSCODE_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["NEXUSCODE_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://opencode.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["NEXUSCODE_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["NEXUSCODE_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in NEXUSCODE_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["NEXUSCODE_CONFIG_CONTENT"]
